...
export default function Matchmaking() {
  ...
  // Auto-start search once userId is set
  useEffect(() => {
    if (userId && !searching) {
      setSearching(true);
      startSearch(userId);
  ...
  async function startSearch(uid: string) {
    // Look for open waiting room from someone else
  ...
  async function cancelSearch() {
    if (pollRef.current) clearInterval(pollRef.current);
    if (userId) {
      await supabase.from("rooms").delete().eq("player1_id", userId).eq("status", "waiting");
    }
    router.push("/");
  }
