...
// Hiragana to romaji conversion table
...
export function checkAnswer(input: string, answers: string[]): boolean {
  ...
  return answers.some(a => {
    ...
    if (clean === aClean) return true;
    // Also accept romaji equivalent of hiragana answer
    const aRoma = hiraToRoma(aClean);
    if (clean === aRoma) return true;
    // And accept hiragana if user typed romaji answer that matches
    const inputRoma = hiraToRoma(clean);
    if (inputRoma === aClean) return true;
    return false;
  });
}
